...
install_nvidia_debian() {
    ...
    if echo "$debian_version" | grep -qi "testing"; then
        echo "${YELLOW}⚠ Você está usando o Debian Testing${NC}"
        echo "${YELLOW}A NVIDIA pode não ter suporte oficial para o Testing${NC}"
        echo ""
        ...
        if [ -n "$user_version" ]; then
            ...
            echo "${YELLOW}⚠ O download pode falhar se o Testing não tiver suporte oficial${NC}"
        fi
    else
