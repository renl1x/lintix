...
import_mok_key() {
    if ! command -v mokutil &>/dev/null; then
        echo "${YELLOW}mokutil não instalado. Pulando importação...${NC}"
        return 1
    fi
    
    if ! sudo mokutil --sb-state 2>/dev/null | grep -qi "SecureBoot enabled"; then
        echo "${YELLOW}Secure Boot não está ativo. Pulando importação...${NC}"
        return 1
    fi
    
    if [ ! -f /var/lib/dkms/mok.pub ]; then
        echo "${YELLOW}Arquivo /var/lib/dkms/mok.pub não encontrado. Pulando importação...${NC}"
        return 1
    fi
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "${YELLOW}⚠ IMPORTANTE: Você precisa reiniciar o sistema para concluir o enrollment da chave MOK${NC}"
        echo "${YELLOW}Durante a reinicialização, siga as instruções na tela para confirmar a senha${NC}"
        echo "${YELLOW}Após confirmar, o Secure Boot será ativado e o driver NVIDIA será carregado${NC}"
    else
        echo "${RED}✗ Falha ao importar a chave MOK${NC}"
        return 1
    fi
}
...
install_nvidia_debian() {
    ...
    if echo "$debian_version" | grep -qiE "testing|sid"; then
        echo "${YELLOW}⚠ Você está usando o Debian Testing ou Sid (Unstable)${NC}"
        echo "${YELLOW}A NVIDIA pode não ter suporte oficial para estas versões${NC}"
        echo ""
        echo "${CYAN}Digite a versão do Debian que você quer usar para o driver NVIDIA:${NC}"
        echo "  Exemplos: 12 (bookworm), 11 (bullseye), 10 (buster)"
        echo "  Ou pressione Enter para usar a versão detectada (${debian_version})"
        read -p "Versão do Debian: " user_version
        
        if [ -n "$user_version" ]; then
            debian_version="$user_version"
            echo "${YELLOW}Usando Debian ${debian_version} para o driver NVIDIA${NC}"
        else
            echo "${YELLOW}Usando versão detectada: ${debian_version}${NC}"
            echo "${YELLOW}⚠ O download pode falhar se Testing/Sid não tiver suporte oficial${NC}"
            return 1
        fi
    else
    ...
    echo "${YELLOW}Baixando driver NVIDIA para Debian ${debian_version}...${NC}"
