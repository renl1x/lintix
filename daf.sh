...
detect_secureboot_support() {
    ...
    if [ -d /sys/firmware/efi ]; then
        ...
        if command -v mokutil &>/dev/null; then
            ...
            echo "${YELLOW}⚠ mokutil não instalado, não foi possível verificar Secure Boot${NC}"
        fi
    else
...
install_nvidia_with_mok() {
    ...
    if echo "$debian_version" | grep -qiE "testing|sid"; then
        ...
        if [ -n "$user_version" ]; then
            ...
            echo "${RED}⚠ Você precisa escolher uma versão estável para o driver NVIDIA${NC}"
            echo "${RED}O driver NVIDIA não tem suporte oficial para Testing/Sid${NC}"
            return 1
        fi
    else
    ...
    echo "${YELLOW}Configurando Secure Boot para o driver NVIDIA...${NC}"
    
    if ! command -v mokutil &>/dev/null; then
        echo "${YELLOW}Instalando mokutil...${NC}"
        sudo apt install -y mokutil
    fi
    
    if [ ! -f /var/lib/dkms/mok.pub ]; then
        echo "${YELLOW}Arquivo /var/lib/dkms/mok.pub não encontrado. Tentando gerar...${NC}"
    ...
    if [ -f /var/lib/dkms/mok.pub ]; then
        if sudo mokutil --list-enrolled 2>/dev/null | grep -q "Debian Secure Boot"; then
            ...
            if [ $? -eq 0 ]; then
                ...
                echo "${YELLOW}  3. Após confirmar, ative o Secure Boot na BIOS/UEFI${NC}"
                echo "${YELLOW}  4. O driver NVIDIA será carregado corretamente${NC}"
            else
                echo "${RED}✗ Falha ao importar a chave MOK${NC}"
                echo "${YELLOW}Você pode tentar importar manualmente depois com:${NC}"
                echo "  sudo mokutil --import /var/lib/dkms/mok.pub"
            fi
        fi
    else
        echo "${RED}✗ Arquivo /var/lib/dkms/mok.pub não encontrado${NC}"
        echo "${YELLOW}Certifique-se de que o pacote dkms está instalado e tente novamente${NC}"
    fi
}
